...
t_philo	*create_node(char **argv, int i)
{
	...
	sem_unlink(ft_strjoin("/last_meal_p", ft_itoa(new_node->philo_id)));
	new_node->last_meal_sem = sem_open(ft_strjoin("/last_meal_p", ft_itoa(new_node->philo_id)), O_CREAT, 0644, 1);
...
void	ft_free_list(t_philo **philo)
{
	...
	{
		sem_close(temp->last_meal_sem);
		sem_close(temp->eat_count_sem);
		temp = (*philo)->next;
		free(*philo);
		*philo = temp;
	}
