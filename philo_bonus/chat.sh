#!/bin/bash
trap 'exit 130' INT # allows exit with Ctrl-C
...
# -------------------- COMMANDS CHECK --------------------
commands_needed=("awk" "echo" "date" "sed" "pgrep" "sleep" "valgrind" "git" "mkdir" "make" "nm" "grep" "wc" "cat" "timeout" "ps" "hostname" "head" "bc" "pkill")
for cmd in "${commands_needed[@]}"; do
    command -v $cmd > /dev/null 2>&1 || fatal_error "'$cmd' command not installed... Aborting."
done

# -------------------- ARGUMENTS CHECK --------------------
if [ "$#" -ne 2 ]; then
    echo -e "${yellow}Usage: start.sh <Project Folder> <Test Type>"
...
error_log() {
    ...
    local log_file="./error_${test_name}_test${test_num}_${datetime}.log"
    echo "[$test_name-$test_num]: $message" > "$log_file"
}
...
generate_log_file() {
    ...
    local test_number="$2"
    echo "./log_${program_name}_test${test_number}_$(date +%s).log"
}
...
test_philosopher_death() {
    ...
    local params=("${@:3:4}")
    local test_number="${7}"
    local log_file=$(generate_log_file "$program_name" "$test_number")

    echo -e "${yellow}[+] Testing $program_name with ${params[@]}${reset}"
    timeout 10 "$program_path/$program_name" "${params[@]}" > "$log_file" 2>&1
    ...
    test_valgrind "$program_name" "$program_path" "${params[@]}" "$test_number"
    test_helgrind "$program_name" "$program_path" "${params[@]}" "$test_number"

    if check_death_occurred "$test_number" "$log_file" && check_simulation_ends "$test_number" "$log_file"; then
        rm -f "$log_file"
    else
        echo "${red}[-] Test #${test_number} Failed: Log kept at $log_file${reset}"
    fi
}
...
death_timing() {
    ...
    while read -r line; do
        ...
        if [[ $line == *"died"* ]]; then
            local error=$((timestamp - time_to_die))
            if (( timestamp < time_to_die || timestamp > (time_to_die + 10) )); then
                echo "${red}[-] Test #${test_num} Error: Timestamp incorrect ($timestamp, error $error ms)${reset}"
            else
                echo "${green}[+] Test #${test_num} Timestamp correct (error $error ms)${reset}"
            fi
            break
        fi
    done < "$file"
...
test_philosopher_meals() {
    ...
    timeout 20 "$program_path/$program_name" "${program_params[@]}" > "$log_file" 2>&1 &

    while [ $i -lt 20 ]; do
        pgrep "$program_name" > /dev/null || break
        sleep 1
        i=$((i + 1))
    done

    check_philosophers_eat "$log_file" "$3" "$7" "$test_number" "${program_params[@]}"
    test_valgrind_meals "$program_name" "$program_path" "${program_params[@]}" "$test_number"
    test_helgrind_meals "$program_name" "$program_path" "${program_params[@]}" "$test_number"
...
check_death_occurred() {
    ...
    echo "${green}[+] Test #${test_num} Succeeded: Only one death occurred${reset}"
}
...
check_simulation_ends() {
    ...
    local log_file="$2"
    local after_death
    after_death=$(sed -n '/died/{n;p;}' "$log_file")

    if [ -n "$after_death" ]; then
        echo "${red}[-] Test #${test_num} Failed: Simulation continues after death${reset}"
        return 1
    fi
...
check_philosophers_eat() {
    ...
    for ((i=1; i<=num_philosophers; i++)); do
        local count
        count=$(grep -w -c "$i is eating" "$log_file")
        echo -e "${yellow}[+] Philosopher $i ate $count times${reset}"
        if [ "$count" -lt "$num_meals" ] || [ "$count" -gt $((num_meals + 2)) ]; then
            echo "${red}[-] Test #${test_num} Failed: Philosopher $i incorrect meal count${reset}"
            echo "${red}[-] Parameters: ${parameters[@]}${reset}"
            return 1
        fi
    done
    echo "${green}[+] Test #${test_num} Succeeded: All philosophers ate correctly${reset}"
}
...
test_valgrind() {
    ...
    shift 2
    local parameters=("$@")
    local test_num="${parameters[-1]}"
    unset parameters[-1]

    local valgrind_log="./valgrind_${program_name}_test${test_num}_$(date +%s).log"
    timeout 30 valgrind --leak-check=full --errors-for-leak-kinds=all --error-exitcode=1 "$program_path/$program_name" "${parameters[@]}" &> "$valgrind_log"
    if [ $? -eq 0 ]; then
        echo "${green}[+] Test #${test_num} Valgrind Passed${reset}"
        rm -f "$valgrind_log"
    else
        echo "${red}[-] Test #${test_num} Valgrind Failed: Memory leaks detected. Log: $valgrind_log${reset}"
    fi
}
...
test_helgrind() {
    ...
    local helgrind_log="./helgrind_${program_name}_test${test_num}_$(date +%s).log"
    timeout 30 valgrind --tool=helgrind --error-exitcode=1 "$program_path/$program_name" "${parameters[@]}" &> "$helgrind_log"
    if [ $? -eq 0 ]; then
        echo "${green}[+] Test #${test_num} Helgrind Passed${reset}"
        rm -f "$helgrind_log"
    else
        echo "${red}[-] Test #${test_num} Helgrind Failed: Race conditions detected. Log: $helgrind_log${reset}"
    fi
}
