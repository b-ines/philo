...
test_valgrind () {
  ...
  local test_num="$8"
  valgrind_log="./valgrind_$1.log"
timeout 30 valgrind --leak-check=full --errors-for-leak-kinds=all --error-exitcode=1 "$2/$1" $parameters &> "$valgrind_log"
if [ $? -eq 0 ]; then
  echo "${green}[+] Test #${test_num} Valgrind Test Succeeded !${reset}"
  rm -f "$valgrind_log"   # supprime seulement si OK
else
  echo "${red}[-] Test #${test_num} Valgrind Test Failed: Memory leaks detected. Log kept at $valgrind_log${reset}"
fi
}
...
test_helgrind () {
  local parameters="$3 $4 $5 $6 $7"
  local test_num="$8"
  helgrind_log="./helgrind_$1.log"
timeout 30 valgrind --tool=helgrind --error-exitcode=1 "$2/$1" $parameters &> "$helgrind_log"
if [ $? -eq 0 ]; then
  echo "${green}[+] Test #${test_num} Helgrind Test Succeeded !${reset}"
  rm -f "$helgrind_log"
else
  echo "${red}[-] Test #${test_num} Helgrind Test Failed: Race conditions detected. Log kept at $helgrind_log${reset}"
fi
